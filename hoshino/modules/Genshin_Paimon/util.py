...
async def get_use_cookie(user_id, uid='', mys_id='', action=''):
    ...
    if not cookies:
        ...
        if not public_cookie:
            logger.info('---派蒙当前没有可用的公共cookie，可能是都达到了上限或没有公共cookie---')
            return None
        else:
...
async def get_at_target(msg):
    return next(
        (msg_seg.data['qq'] for msg_seg in msg if msg_seg.type == "at"), None
    )
        ...
        for superuser in get_bot().config.SUPERUSERS:
            try:
                await get_bot().send_private_msg(
                    user_id=int(superuser), message=f'{msg}，派蒙帮你删除啦!'
                )
